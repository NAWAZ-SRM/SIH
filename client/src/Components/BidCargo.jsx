                    ...
                    {cargoList.length > 0 ? (
                        cargoList.map(cargo => (
                            <li key={cargo._id}>
                                ...
                                <p>Estimated Price: ${cargo.estimatedPrice}</p> 
...
<ul className="flex flex-wrap gap-6 p-4 bg-gray-900">
    {cargoList.length > 0 ? (
        cargoList.map(cargo => (
            <li key={cargo._id} className="text-center flex-none w-full sm:w-64 bg-white p-4 rounded-lg shadow border border-gray-200">
                <h3 className="text-xl font-semibold text-[#394855] mb-2">{cargo.title}</h3>
                <p className="text-gray-700 mb-1">{cargo.description}</p>
                <p className="text-gray-600 mb-1">Weight: {cargo.weight}</p>
                <p className="text-gray-600 mb-1">Hazardous: {cargo.isHazardous ? 'Yes' : 'No'}</p>
                <p className="text-gray-600 mb-2">Estimated Price: ${cargo.estimatedPrice}</p>
                <button 
                    onClick={() => handleCargoSelect(cargo._id)}
                    className="px-4 py-2 bg-[#394855] text-white rounded-lg shadow hover:bg-[#2e3b43] transition duration-300"
                >
                    View and Bid
                </button>
            </li>
        ...
        <p className="text-gray-500">No cargo available</p>
            ...
            {selectedCargo && (
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm mb-4">
                    <h2 className="text-xl font-semibold text-[#394855] mb-2">Place a Bid for {selectedCargo.title}</h2>
                    <BidForm userdata={userdata} cargoId={selectedCargo._id} />
                </div>
            
                <div className="bg-gray-900 p-4 rounded-lg shadow-sm text-white">
                    <h2 className="text-xl font-semibold text-white mb-2">Bids</h2>
